...
// `enabled` lets callers (e.g. the dashboard) skip this request entirely for
// roles without PHARMACY.VIEW. Without it, the backend 403s, the query's
// `data` stays undefined, and `lowStock?.length ?? 0` renders as a misleading
// "0 — all clear" instead of simply not showing pharmacy stock info to a role
// that has no business seeing it.
export function useLowStockMedicines(enabled = true) {
  return useQuery({
...
export function useExpiringBatches(days = 30, enabled = true) {
  return useQuery({
