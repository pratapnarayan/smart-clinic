...
export function EmployeeFormModal({ open, onClose, employee }: Props) {
  ...
  // Controlled open-state for the two DatePickers below so we can force them
  // to close immediately on selection (left uncontrolled, antd sometimes left
  // the calendar open after picking a date, requiring an extra click elsewhere
  // to dismiss it).
          ...
          <Col span={12}>
            <Form.Item name="_joinDate" label="Join Date" rules={[{ required: true }]}>
              <DatePicker
                ...
                open={joinDateOpen}
                onOpenChange={setJoinDateOpen}
                onChange={() => setJoinDateOpen(false)}
              />
            </Form.Item>
          </Col>
          <Col span={12}>
